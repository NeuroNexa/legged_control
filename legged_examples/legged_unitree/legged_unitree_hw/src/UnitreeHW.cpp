...
// Conditional compilation for different SDK versions.
...
bool UnitreeHW::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh) {
  ...
  // Set up the hardware interfaces for joints, IMU, and contact sensors.
  setupJoints();
  setupImu();
  setupContactSensor(robot_hw_nh);

  // Initialize the Unitree SDK's UDP communication.
  // The SDK version determines the correct constructor to use.
#ifdef UNITREE_SDK_3_3_1
  ...
  // Initialize the SDK's safety layer based on the robot type.
...
void UnitreeHW::read(const ros::Time& time, const ros::Duration& /*period*/) {
  // Receive the latest state packet from the robot via UDP.
  udp_->Recv();
  udp_->GetRecv(lowState_);

  // Unpack the state packet into the data buffers for the ros_control interfaces.
  // Joints
  for (int i = 0; i < 12; ++i) {
  ...
  // Contact sensors (based on foot force)
  for (size_t i = 0; i < CONTACT_SENSOR_NAMES.size(); ++i) {
    contactState_[i] = lowState_.footForce[i] > contactThreshold_;
  }

  // Set default commands to prevent oscillation when no controller is running.
  // This effectively makes the robot compliant.
  for (const auto& name : hybridJointInterface_.getNames()) {
    HybridJointHandle handle = hybridJointInterface_.getHandle(name);
    handle.setFeedforward(0.);
    handle.setVelocityDesired(0.);
    handle.setKd(3.0); // A small amount of damping
  }
}

void UnitreeHW::write(const ros::Time& /*time*/, const ros::Duration& /*period*/) {
  // Pack the commands from the ros_control interfaces into the SDK's command struct.
  for (int i = 0; i < 12; ++i) {
  ...
  // Apply the SDK's safety checks (e.g., position and power limits).
  safety_->PositionLimit(lowCmd_);
  safety_->PowerProtect(lowCmd_, lowState_, powerLimit_);

  // Send the command packet to the robot via UDP.
  udp_->SetSend(lowCmd_);
  udp_->Send();
}

bool UnitreeHW::setupJoints() {
  // Map the joint names from the URDF to the indices used by the Unitree SDK.
  for (const auto& joint : urdfModel_->joints_) {
    ...
    if (joint.first.find("HAA") != std::string::npos) { // Hip Abduction/Adduction
      joint_index = 0;
    } else if (joint.first.find("HFE") != std::string::npos) { // Hip Flexion/Extension
      joint_index = 1;
    } else if (joint.first.find("KFE") != std::string::npos) { // Knee Flexion/Extension
      joint_index = 2;
    } else {
      continue;
    }

    // Register the joint state and hybrid command handles.
...
bool UnitreeHW::setupImu() {
  // Register the IMU sensor handle.
  ...
  // Set some default covariance values.
...
bool UnitreeHW::setupContactSensor(ros::NodeHandle& nh) {
  // Get the contact force threshold from the parameter server.
  nh.getParam("contact_threshold", contactThreshold_);
  // Register the contact sensor handles.
  for (size_t i = 0; i < CONTACT_SENSOR_NAMES.size(); ++i) {
